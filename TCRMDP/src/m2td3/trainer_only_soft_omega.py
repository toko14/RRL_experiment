...
from m2td3.algo_only_soft_omega import M2TD3OnlySoftOmega, M2TD3Config
...
class Trainer:
    ...
    def __init__(
        ...
        **kwargs: Unpack[M2TD3Config],
        ...
        self.data_collection_csv_path = (
            f"{self.output_dir}/metrics/uncertainty_data_collection.csv"
        )
        self.training_update_csv_path = (
            f"{self.output_dir}/metrics/uncertainty_training_update.csv"
        )
    ...
    def _get_current_omega_temperature(self, step: int) -> float:
        ...
        if progress <= warmup_end:
            ...
            return self.omega_temperature_start + decay_progress * (
                self.omega_temperature_end - self.omega_temperature_start
            )
        else:
        ...
        while not done and not truncated:
            ...
            if self.step >= self.start_steps:
                ...
                if (
                    ...
                    header = [
                        "step",
                        "worst_policy_loss_value",
                        "worst_policy_loss_index",
                        "soft_policy_loss_value",
                        "soft_omega_loss_value",
                    ]
                    header.extend(
                        [
                            f"update_omega_{i}"
                            for i in range(len(stats["update_omega"]))
                        ]
                    )
                    header.extend(
                        [
                            f"hatomega_prob_{i}"
                            for i in range(len(stats["hatomega_prob"]))
                        ]
                    )
                    row = [
                        self.step,
                        stats["worst_policy_loss_value"],
                        stats["worst_policy_loss_index"],
                        stats.get("soft_policy_loss_value", ""),
                        stats.get("soft_omega_loss_value", ""),
                    ]
    ...
    def main(self):
        ...
        while True:
            ...
            flag, total_reward = self.interact(
                self.env, omega, dis_restart_flag, prob_restart_flag
            )
