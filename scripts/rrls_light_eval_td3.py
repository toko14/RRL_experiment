...
def build_agent(policy_path: str, env: gym.Env, device: str, tcrmdp_src: str | None):
    # TD3 側もパッケージ import を行うと td3/__init__.py 経由で trainer (wandb 依存) を読み込みにいく。
    # 評価では不要なので、models.py をファイル直指定の動的 import に切り替える。
    ...
    models_path = os.path.join(tcrmdp_src, "td3", "models.py")
    if not os.path.exists(models_path):
        raise FileNotFoundError(f"td3/models.py not found: {models_path}")

    spec = spec_from_file_location("td3_models", models_path)
    ...
    # Instantiate actor with current env action space (buffers action_scale/bias will align)
    actor = Actor(observation_dim=obs_dim, action_space=env.action_space).to(device_t)

    # Load checkpoint (either raw state_dict or TD3 checkpoint with "actor" key)
    ckpt = torch.load(policy_path, map_location="cpu")
    state = ckpt["actor"] if isinstance(ckpt, dict) and "actor" in ckpt else ckpt
    actor.load_state_dict(state)
    actor.to(device_t)

    return TD3ActorWrapper(actor=actor, device=device_t, obs_dim=obs_dim)
...
def main():
    parser = argparse.ArgumentParser(description="Lightweight RRLS evaluation for TD3 policies (~100k steps)")
    ...
    print("==== Lightweight RRLS evaluation (TD3) ====")
